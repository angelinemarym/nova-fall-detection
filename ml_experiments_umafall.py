...
print(f"[UMAFall ML] Using random seed: {SEED}", flush=True)

# ==========================================
# 1. Configuration
# ==========================================
DATA_DIR = './processed_tensors_umafall_no_hr'
OUTPUT_DIR = f'./results_ml_umafall/seed_{SEED}'
...
def get_model_size_kb(model_name, model, X_train, y_train):
    ...
    if model_name == 'kNN':
        ...
        temp_path = 'temp_model_umafall.pkl'
...
def run_experiments():
    ...
    with open(os.path.join(OUTPUT_DIR, 'summary_results.txt'), 'w') as f:
        f.write("UMAFall ML Experiment Results Summary\n")
        f.write("="*50 + "\n")
